...
const DataSonification = () => {
  ...
  const presetData = {
    carbonEmissions: {
      ...
      data: [358.5, 511.7, 666.2, 827.3, 930.2, 1371.5, 1856.8, 2551.6, 2743.4, 2897.3, 3060.2, 2990.7],
      multiplier: 1,
      baseFreq: 0
    },
    pi: {
  ...
  const findNearestNote = (freq) => {
    ...
    const cents = 1200 * Math.log2(freq / (standardNotes[nearestNote] * Math.pow(2, octave - 4)));
  ...
  const playNote = (value) => {
    ...
    try {
      ...
      gainNode.gain.setValueAtTime(sustainLevel, currentTime + attackTime + decayTime + sustainTime);
      // Release
  ...
  const handleSaveData = () => {
    try {
      ...
      if (Array.isArray(newData) && newData.every(num => typeof num === 'number')) {
        setData(newData);
        return;
      }
    } catch (err) {
  ...
  const adjustADSR = (newValue, parameter) => {
    ...
    const others = ['attack', 'decay', 'sustain', 'release'].filter(p => p !== parameter);
    const currentSum = others.reduce((sum, p) => {
      return sum + (p === 'attack' ? attack :
                   p === 'decay' ? decay :
                   p === 'sustain' ? sustain :
                   release);
    }, 0);
    ...
    <div className="flex flex-col items-center p-8">
      ...
      <div className="w-full max-w-md space-y-4 mb-8">
        <div className="flex flex-wrap gap-2">
          <label className="w-full block text-sm font-medium mb-1">Preset Data</label>
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Data Array (JSON format)</label>
          <textarea
            value={dataInput}
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Note Duration (ms)</label>
          <input
            type="number"
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs">Attack ({(attack * 100).toFixed(0)}%)</label>
              <input
                type="range"
                min="0"
                max="1"
                step="0.01"
                value={attack}
                onChange={(e) => adjustADSR(parseFloat(e.target.value), 'attack')}
                className="w-full"
              />
            </div>

            <div>
              <label className="block text-xs">Decay ({(decay * 100).toFixed(0)}%)</label>
              <input
                type="range"
                min="0"
                max="1"
                step="0.01"
                value={decay}
                onChange={(e) => adjustADSR(parseFloat(e.target.value), 'decay')}
                className="w-full"
              />
            </div>

            <div>
              <label className="block text-xs">Sustain ({(sustain * 100).toFixed(0)}%)</label>
              <input
                type="range"
                min="0"
                max="1"
                step="0.01"
                value={sustain}
                onChange={(e) => adjustADSR(parseFloat(e.target.value), 'sustain')}
                className="w-full"
              />
            </div>

            <div>
              <label className="block text-xs">Release ({(release * 100).toFixed(0)}%)</label>
              <input
                type="range"
                min="0"
                max="1"
                step="0.01"
                value={release}
                onChange={(e) => adjustADSR(parseFloat(e.target.value), 'release')}
                className="w-full"
              />
            </div>

            <div>
              <label className="block text-xs">Sustain Level ({(sustainLevel * 100).toFixed(0)}%)</label>
              <input
                type="range"
                min="0"
                max="1"
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Time Between Notes (ms)</label>
          <input
            type="number"
