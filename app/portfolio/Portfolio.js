...
const ExternalApp = ({ app }) => {
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-md overflow-hidden transition-all lg:min-h-[60vh] duration-300`}
    >
      <div className="p-6">
      ...
      {!includesKeyword && !app.hide && (
        <div className={`transition-all duration-300 h-[400px] sm:h-[600px]`}>
          <div
            className={`w-full h-full flex items-center justify-center bg-gray-100 ${isLoading ? "block" : "hidden"}`}
          >
...
const InternalApp = ({ app }) => {
  const AppComponent = dynamic(
    () => import(`../components/internalApps/${app.component}`),
    {
      loading: () => <p>Loading...</p>,
      ssr: false,
    },
...
const Portfolio = () => {
  ...
  const sortedExternalApps = externalApps.sort(
